...
// IsWritable reports whether the shard still accepts new edges.
func (s *Shard) IsWritable() bool {
...
// checkWritable enforces the shard state machine: only active generations
// accept new edges, so a stale route cannot leak writes into a frozen one.
func (s *Shard) checkWritable() error {
	if s.State != Active {
		return fmt.Errorf("%w: shard %d is %s (writable=%t)", model.ErrShardSealed, s.ID, s.State, s.IsWritable())
	}
	return nil
}
