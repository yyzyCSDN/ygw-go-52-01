...
func (s *Store) DeleteVertex(vertexID string) error {
	...
	for _, edge := range incident {
		if err := s.removeEdgeCleanup(edge); err != nil {
			return fmt.Errorf("store: cleanup edge %q for vertex %q: %w", edge.ID, vertexID, err)
		}
	}
	delete(s.vertices, vertexID)
	s.vertexSeq = removeFromSlice(s.vertexSeq, vertexID)
	if err := s.attrIndex.RemoveVertex(vertexID); err != nil {
		return err
	}
	if vertex.Label != "" {
