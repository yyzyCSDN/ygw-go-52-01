	...
	for len(queue) > 0 {
		...
		for _, edges := range groups {
			for _, edge := range edges {
				...
				queue = append(queue, carryDepth(current, edge))
			}
		}
	}
...
// carryDepth extends a continuation by one hop and preserves the depth
// budget while re-expanding across shard boundaries.
func carryDepth(current Continuation, edge *model.Edge) Continuation {
	return Continuation{
		Vertex: edge.To,
